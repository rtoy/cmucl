...
;;;
(ext:file-comment
 "$Header: /Volumes/share2/src/cmucl/cvs2git/cvsroot/src/compiler/x86/alloc.lisp,v 1.13.36.1 2010/02/08 17:15:51 rtoy Exp $")
;;;
;;; **********************************************************************
;;;
...
(define-vop (make-symbol)
  ...
  (:generator 37
    (with-fixed-allocation (result symbol-header-type symbol-size node)
      ...
      (load-foreign-data-symbol state-addr "fast_random_state")
      (inst imul temp
	    (make-ea :dword :base state-addr)
	    1103515245)
      (inst add temp 12345)
      (inst mov (make-ea :dword :base state-addr) temp)
