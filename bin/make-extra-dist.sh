...
installit -d -m 0755 "$DESTDIR"/"$CMUCLLIBVER"/lib/contrib

for ext in $FASL
do
  installit -m 0644 "$TARGET"/clx/clx-library."$ext" \
      "$DESTDIR"/"$CMUCLLIBVER"/lib/subsystems/
  installit -m 0644 "$TARGET"/hemlock/hemlock-library."$ext" \
      "$DESTDIR"/"$CMUCLLIBVER"/lib/subsystems/
  installit -m 0644 "$TARGET"/interface/clm-library."$ext"  \
      "$DESTDIR"/"$CMUCLLIBVER"/lib/subsystems/
done

installit -m 0644 src/hemlock/XKeysymDB \
	src/hemlock/hemlock11.cursor src/hemlock/hemlock11.mask \
	"$TARGET"/hemlock/spell-dictionary.bin \
	"$DESTDIR"/"$CMUCLLIBVER"/lib/
installit -m 0755 src/hemlock/mh-scan "$DESTDIR"/"$CMUCLLIBVER"/lib/
installit -m 0755 "$TARGET"/motif/server/motifd \
	"$DESTDIR"/"$CMUCLLIBVER"/lib/
...
# these directories.
for d in $( (cd src || exit 1; find contrib -type d -print | grep -v "asdf\|defsystem") )
do
    installit -d -m 0755 "$DESTDIR"/"$CMUCLLIBVER"/lib/"$d"
done

for f in $( (cd src/contrib || exit 1; find . -type f -print | grep -v "asdf\|defsystem\|unix") )
do
    DIR=$(dirname "$f")
    installit -m 0644 src/contrib/"$f" "$DESTDIR"/"$CMUCLLIBVER"/lib/contrib/"$DIR"
done

# Install all the locale data.

for d in $( (cd src/i18n/ || exit 1; find locale -type d -print) )
do
    installit -d -m 0755 "$DESTDIR"/"$CMUCLLIBVER"/lib/"$d"
done

# Install mo files.  Ignore any emacs-style backup files.
for f in $( (cd "$TARGET"/i18n || exit 1; find locale -type f -print | grep -v '~.*~\|.*~') )
do
    DIR=$(dirname "$f")
    installit -m 0644 "$TARGET"/i18n/"$f" "$DESTDIR"/"$CMUCLLIBVER"/lib/"$DIR"
done
