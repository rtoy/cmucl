...
;;;
(ext:file-comment
  "$Header: src/compiler/arm64/move.lisp $")
;;;
;;; **********************************************************************
;;;
;;;    This file contains the ARM64 VM definition of operand loading/saving
;;; and the Move VOP.
;;;
;;; Written by Rob MacLachlan.
;;; SPARC conversion by William Lott.
;;; ARM64 conversion by <contributors>.
;;;
(in-package "ARM64")
...
(define-vop (move-from-signed)
  ...
  (:generator 20
    ...
    (move x arg)
    (let ((done (gen-label)))
      ;; ASR x by positive-fixnum-bits (29): result is 0 for non-negative
      ;; fixnums, -1 (all ones) for negative fixnums, anything else for
      ;; non-fixnums.  EOR temp with (temp ASR 63) maps both 0 and -1
      ;; to 0 and all other values to non-zero.  CBZ branches if zero.
      (inst lsl y x fixnum-tag-bits)
      (inst asr temp x positive-fixnum-bits)
      (inst eor temp temp (shift temp :asr 63))
      (inst cbz temp done)
...
(define-vop (move-from-unsigned)
  ...
  (:generator 20
    ...
    (let ((done (gen-label))
	  (one-word (gen-label)))
      ;; Check if the value fits in a 30-bit unsigned fixnum, i.e. that
      ;; bits 63:30 are all zero.  The mask (lognot (1- (ash 1 (- word-bits
      ;; fixnum-tag-bits)))) = #xFFFFFFFFC0000000 is a contiguous run of 1s
      ;; encodable as an ARM64 logical immediate.
      (inst tst x (lognot (1- (ash 1 (- vm:word-bits fixnum-tag-bits)))))
      (inst lsl y x fixnum-tag-bits)
      (inst b :eq done)
