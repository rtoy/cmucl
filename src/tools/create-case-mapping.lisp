...
(defun dump-case-mapping-table (stream table stage2-size case-table)
  ...
  (flet ((print-table (header table stream)
	   ...
	   (let ((table-byte (if (eq case-table :lower)
		 ...
		 (pprint-newline :fill stream))
	       ...
	       (format stream "0x~4,'0x" (ldb table-byte (aref table n))))
	     (princ #\, stream)
	     (pprint-newline :mandatory stream))))
    (let ((index 0)
	  offsets)
...
(defun dump-case-mapping-index (stream table stage2-size)
 ...
 */~2%"
      ...
      (dump-case-mapping-table s table stage2-size :lower)
      (dump-case-mapping-table s table stage2-size :upper)
      (dump-case-mapping-index s table stage2-size)
      (format s "~

/*
 * Needed for building from a lisp that has the variable stage2 defined.
 */
const uint32_t stage2[];
"))
    (format t "Wrote ~S~%" pathname)))
