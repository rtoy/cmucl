...
;;;
(ext:file-comment
  "$Header: /Volumes/share2/src/cmucl/cvs2git/cvsroot/src/code/string.lisp,v 1.12.30.9 2009/05/02 11:54:37 rtoy Exp $")
;;;
;;; **********************************************************************
;;;
...
(defun decompose (string &optional (compatibility t))
  ...
  (let ((result (make-string (cond ((< (length string) 40)
	...
	(fillptr 0)
	(start 0))
    (declare (type kernel:index fillptr start))
    (labels ((rec (string)
	     ...
	     (out (code)
	       (when (zerop (unicode-combining-class code))
		 (order result start fillptr)
		 (setq start fillptr))
	       (multiple-value-bind (hi lo) (surrogates code)
		 (outch hi)
		 (when lo
		 ...
		 (let ((tmp (make-string (round (length result) 5/6))))
	       ...
	       (incf fillptr))
	     (order (string start end)
	       (when (> (- end start) (if (zerop start) 1 2))
		 (loop for done = t do
		      (do ((x start (1+ x)))
			  ((>= x (1- end)))
			(declare (type kernel:index x))
			(multiple-value-bind (code1 wide1)
			    (codepoint string x)
			  (when (and wide1 (= (+ x 2) end)) (return))
			  (multiple-value-bind (code2 wide2)
			      (codepoint string (+ x (if wide1 2 1)))
			    (when (> (unicode-combining-class code1)
				     (unicode-combining-class code2))
			      (rotatef wide1 wide2)
			      (case (+ (if wide1 2 0) (if wide2 1 0))
				(0 (rotatef (schar string x)
					    (schar string (+ x 1))))
				(1 (rotatef (schar string x)
					    (schar string (+ x 2))
					    (schar string (+ x 1))))
				(2 (rotatef (schar string x)
					    (schar string (+ x 1))
					    (schar string (+ x 2))))
				(3 (rotatef (schar string x)
					    (schar string (+ x 2)))
				   (rotatef (schar string (+ x 1))
					    (schar string (+ x 3)))))
			      (setq done nil))
			    (when wide1 (incf x)))))
		      until done))))
