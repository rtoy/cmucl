...
;;;
(ext:file-comment
  "$Header: /Volumes/share2/src/cmucl/cvs2git/cvsroot/src/code/string.lisp,v 1.19 2009/09/15 15:51:25 rtoy Exp $")
;;;
;;; **********************************************************************
;;;
...
(defun string-next-word-break (s i)
  (let ((n (length s)))
	...
	((char-word-break-category (c)
	   ...
	   ;; algorithm wants.
	   (let ((cat (lisp::unicode-word-break (char-code c))))
	     (case cat
	       ((:lf :cr :newline)
		:sep)
	       ((:extend :format)
	 ...
	 (lookup (j context)
      ...
      (cond ((< i 0) 0)
	    ((<= n i) n)
	    (t (lookup (+ i 1) (left-context i)))))))

(defun string-capitalize-unicode (s &key (start 0) end casing)
  ...
  (let ((end (or end (length s))))
    (with-output-to-string (result)
      (let ((upper (ecase casing
		     (:simple
		      #'(lambda (ch)
			  (write-char (char-upcase ch) result)))
		     (:full
		      #'(lambda (ch)
			  (write-string (unicode-full-case-title (char-code ch)) result))))))
	(when (plusp start)
	  (write-string s result :start 0 :end start))
	(do ((start start next)
	     (next (string-next-word-break s start)
		   (string-next-word-break s next)))
	    ((or (= start next)
		 (>= start end))
	     (write-string s result :start start)
	     nil)
	  (funcall upper (char s start))
	  (write-string (string-downcase s :start (1+ start) :end next :casing casing)
