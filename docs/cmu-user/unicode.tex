...
\begin{defun}{}{char-equal}{\amprest{} \var{characters}}
   ...
   For the comparison, the each character has its case folded
   according to the Unicode simple case folding table and the
   characters are compared based on the character code value.
\end{defun}
...
\begin{defun}{}{name-char}{\args \var{name}}
  ...
  case-sensitive, and spaces or underscores are optional.
\end{defun}
\subsection{Strings}
...
\begin{defun}{}{string-equal}{\args \var{s1} \var{s2} \keys{\kwd{start1}
      \kwd{end1} \kwd{start2} \kwd{end2} \kwd{casing}}}
  \defunx{string-not-equal}{\args \var{s1} \var{s2} \keys{\kwd{start1}
      \kwd{end1} \kwd{start2} \kwd{end2} \kwd{casing}}}
  \defunx{string-lessp}{\args \var{s1} \var{s2} \keys{\kwd{start1}
      \kwd{end1} \kwd{start2} \kwd{end2} \kwd{casing}}}
  \defunx{string-greaterp}{\args \var{s1} \var{s2} \keys{\kwd{start1}
      \kwd{end1} \kwd{start2} \kwd{end2} \kwd{casing}}}
  \defunx{string-not-greaterp}{\args \var{s1} \var{s2}
    \keys{\kwd{start1} \kwd{end1} \kwd{start2} \kwd{end2} \kwd{casing}}}
  \defunx{string-not-lessp}{\args \var{s1} \var{s2} \keys{\kwd{start1} \kwd{end1} \kwd{start2} \kwd{end2}}}
  These functions perform case-insenstive comparisons.  How the
  comparison is done depends on the value of \code{casing}.  The case
  of each codepoint is folded according to the Unicode case folding
  table.  When \code{casing} is \kwd{simple}, simple case folding is
  done, where each codepoint is replaced by a single codepoint.
  Otherwise, full case folding is performed where a codepoint may be
  converted to several codepoints.

  Unicode collation is not performed.
\end{defun}
